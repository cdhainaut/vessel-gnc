...
Commands three heading steps (0 -> +90 -> -120 deg) while a PI speed
controller holds cruise speed. The applied yaw moment lags the command behind
the actuator dynamics (docs/model.md §5). Writes
``results/heading_control.png``.
"""
...
# Heading reference [rad]: 0 until 15 s, +90 deg until 30 s, then -120 deg
# (a 150 deg reversal; an exact 180 deg step is ambiguous for angle wrapping).
...
def main() -> None:
    ...
    commanded = []  # (t, moment) for the applied-vs-commanded overlay
    ...
    # 90% rise time and overshoot of the first step (t = 15 s), measured
    # before the second step (t = 30 s).
    window = (result.t >= 15.0) & (result.t < 30.0)
    ...
    t_cmd, n_cmd = zip(*commanded, strict=True)
    ax_m.plot(t_cmd, n_cmd, "k--", lw=1.0, label="command")
    ax_m.plot(result.t, result.yaw_moment, lw=1.8, label="applied (actuator)")
