...
def _estimator_body(scenario: dict, estimator: dict) -> str:
    ...
    return (
        ...
        f"| Current error RMS [m/s] (after {transient:.1f} s transient) | "
        f"{estimator['current_error_rms_m_s']:.3f} |\n"
        f"| Current error max [m/s] (after {transient:.1f} s transient) | "
        ...
        f"seed {scenario['seed']}). The current estimate absorbs all "
        "unmodelled environmental forces, so wind gusts and model mismatch "
        "can contaminate it: the current error is an equivalent-disturbance "
        "indicator (docs/estimation.md §5), not a standalone current-sensor "
        "validation.\n"
        "\n"
    )
