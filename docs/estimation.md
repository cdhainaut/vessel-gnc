...
State vector (identical to the vessel state, docs/model.md §1):
...
| No-noise consistency | Exact model, near-zero R: estimate converges to the true state (< 1e-3) | Pass |
| Closed-loop tracking | LOS + PID on EKF estimates, 0.15 m/s current + 3 N wind unknown to the filter, 120 s | RMS position error < 3 m, max < 6 m |
...
- The filter does not estimate the current; a steady current appears as a
  ...
  to GNSS). Current estimation is a planned extension.
